...
class Signup(QMainWindow):
    ...
    def save(self): #database
        ...
        pw = self.pw.text()
        username = self.username.text()  # makes username text
        password = self.password.text()  # makes password text
        #  if both boxes are filled
        if username != '' and password != '':  # adds to databse if both boxes are filled
            if password == pw:
                c.execute("INSERT INTO info VALUES(?, ?, ?, ?, ?, ?, ? )", (sname, fname, mname, bday, username, email, password))
...
class userLog(QMainWindow):
    ...
    def login(self):
        ...
        if un == '' or pw == '':  # sends error if a box is empty
            QMessageBox.question(self, 'Error', "One box is empty", QMessageBox.Ok)

        #  if both boxes are filled, searches for the result
        elif un != '' or pw != '':
            c.execute("SELECT * FROM info WHERE username =? AND password =?",(un,pw))
            result = c.fetchone()
            if result == []:  # error if username and password does not match
                QMessageBox.question(self, 'Search Results', "No Record", QMessageBox.Ok)
                self.home = Home()
                win = Home()
                self.close()  

            elif result != []:  # opens options if username and password match
